export interface WebGL extends WebGLRenderingContext {
	program: WebGLProgram;
}

export const getContext = (id: string, vshader: string, fshader: string): WebGL | null => {
	const canvas = document.getElementById(id) as HTMLCanvasElement;
	if (!canvas) {
		console.log("no canvas");
		return null;
	}
	const _gl = canvas.getContext("webgl") as WebGL;
	if (!_gl) {
...
export const initShader = (gl: WebGL, vshader: string, fshader: string): WebGL | null => {
